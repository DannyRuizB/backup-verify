...
#     KEEP_DAYS > 0), "now" being $BV_NOW (epoch seconds) if set - the
#     drills use it to age a backup without waiting a week;
...
retention_victims() {
    ...
    if [ "$keep_days" -gt 0 ]; then
        cutoff=$(date -u -d "@$(( ${BV_NOW:-$(date -u +%s)} - keep_days * 86400 ))" +%Y%m%dT%H%M%SZ)
    fi
